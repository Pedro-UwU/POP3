...
int createTCPSocketServer(const char* service) {
    ...
    if (bind(serverSocket, (struct sockaddr*) &addr, sizeof(addr)) < 0) {
        log(ERROR, "[serverUtils][createTCPSocketServer] Unable to bind socket %d", serverSocket);
        close(serverSocket);
        return -1;
    }
    log(DEBUG, "[serverUtils][createTCPSocketServer] Socket %d bound to port %d", serverSocket, port);

    if (listen(serverSocket, port)) {
        log(ERROR, "[serverUtils][createTCPSocketServer] Unable to listen to port %d with socket %d", port, serverSocket);
        close(serverSocket);
        return -1;
    }
