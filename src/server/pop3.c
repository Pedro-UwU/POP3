...
void pop3AcceptPassive(struct selector_key *key)
{
        ...
        int new_client_socket = accept(key->fd,
        ...
        if (new_client_socket > MAX_SOCKETS) {
                log(INFO, "Socket %d is too hight, closing connection",
                    new_client_socket);
        ...
        client_data *client_data_ptr =
                calloc(1, sizeof(client_data)); // TODO: free
        if (client_data_ptr == NULL) {
                log(ERROR, "New client with socket %d can't alloc client data",
                    new_client_socket);
                close(new_client_socket);
                return;
        }

        init_new_client_data(client_data_ptr, new_client_socket,
                             client_address);

        selector_status status = selector_register(
                key->s, new_client_socket, &pop3_handler,
                OP_WRITE /* it starts with a greeting */, client_data_ptr);

        if (status != SELECTOR_SUCCESS) {
                log(ERROR, "New socket %d registering failed",
                    new_client_socket);
                close(new_client_socket);
        ...
        log(INFO, "New client in socket %d has been registered into seletor",
            new_client_socket);
        return;
}
                                 ...
                                 struct sockaddr_storage client_address)
{
        ...
        buffer_init(&data->read_buffer_client, BUFFER_SIZE,
                    data->read_buffer_data);
        buffer_init(&data->write_buffer_client, BUFFER_SIZE,
