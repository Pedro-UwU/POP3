...
unsigned auth_process(struct selector_key* key) {
    ...
    if (data->is_sending == true) {
        ...
        if (something_to_read == true) {
            log(ERROR, "WTF, is_sending = true and empty buffer in auth_process with socket %d", key->fd);     
    ...
    if (buffer_can_read(output_buffer) == true) {
        log(ERROR, "WTF Should be here if there's data to send in auth_process in socket %d", key->fd);
